...
import { X, Clock, Users, MapPin, AlertTriangle, Brain, TrendingUp, Star, Zap, Target, Award, Calendar, User, Sparkles, Shield, Filter, Search } from 'lucide-react';
...
import { getClassDuration, validateTeacherHours, getClassAverageForSlot, getBestTeacherForClass, getUniqueTeachers, getClassFormatsForDay, isClassAllowedAtLocation, ALL_TIME_SLOTS, getHistoricalClassRows } from '../utils/classUtils';
...
const ClassModal: React.FC<ClassModalProps> = ({
    ...
    try {
      const recommendations = await aiService.generateRecommendations(
        ...
        scheduledClasses,
        {
          ...
          restrictedHours: { start: '12:00', end: '17:00', privateOnly: true },
          weekendStartTimes: { saturday: '08:30', sunday: '10:00' },
  ...
  const getHistoricData = () => {
    ...
    const bestPerformingClass = historicClasses.sort((a, b) => b.checkedIn - a.checkedIn)[0];
    ...
    const minCheckedIn = Math.min(...historicClasses.map(cls => cls.checkedIn));
    ...
    const fullClasses = historicClasses.filter(cls => cls.checkedIn >= 15).length;
    ...
    return {
      ...
      emptyClasses,
      fullClasses,
      successRate: historicClasses.filter(cls => cls.checkedIn > avgCheckedIn).length / historicClasses.length,
      revenuePerParticipant: avgRevenue / avgCheckedIn,
      attendanceRate: checkedInTotal / historicClasses.reduce((sum, cls) => sum + cls.participants, 0),
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className={`border-b ${theme.border}`}>
                      ...
                      <th className={`text-left p-2 ${theme.textSecondary}`}>Unique 1</th>
                      <th className={`text-left p-2 ${theme.textSecondary}`}>Unique 2</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((row, index) => (
                      <tr key={index} className={`border-b ${theme.border} hover:bg-gray-700/30`}>
                        ...
                        <td className={`p-2 ${theme.textSecondary}`}>{row.unique1}</td>
                        <td className={`p-2 ${theme.textSecondary}`}>{row.unique2}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 p-6 rounded-xl border border-blue-500/20">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-3 bg-blue-500/10 rounded-lg">
                      <div className={`text-2xl font-bold ${theme.text}`}>{historicData?.avgCheckedIn.toFixed(1) || 0}</div>
                      <div className="text-xs text-blue-300">Avg Check-ins</div>
                    </div>
                    <div className="text-center p-3 bg-blue-500/10 rounded-lg">
                      <div className={`text-2xl font-bold ${theme.text}`}>₹{Math.round((historicData?.avgRevenue || 0) / 1000)}K</div>
                      <div className="text-xs text-blue-300">Avg Revenue</div>
                    </div>
                    <div className="text-center p-3 bg-blue-500/10 rounded-lg">
                      <div className={`text-2xl font-bold ${theme.text}`}>{historicData?.count || 0}</div>
                      <div className="text-xs text-blue-300">Historic Classes</div>
                    </div>
                    <div className="text-center p-3 bg-blue-500/10 rounded-lg">
                      <div className={`text-2xl font-bold ${theme.text}`}>{Math.round((historicData?.successRate || 0) * 100)}%</div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {aiRecommendations.slice(0, 6).map((rec, index) => (
                      <div
                        ...
                        className={`p-4 ${theme.card} rounded-xl border border-purple-500/30 cursor-pointer hover:border-purple-400 hover:${theme.cardHover} transition-all duration-200 group`}
                      >
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 p-6 rounded-xl border border-blue-500/30">
                <h3 className="font-semibold text-blue-300 mb-4 flex items-center">
                  ...
                  Detailed Historic Data Analysis - {selectedSlot.day} {selectedSlot.time}
                </h3>
                
                {renderHistoricAnalytics()}
              </div>
            </div>
          )}
