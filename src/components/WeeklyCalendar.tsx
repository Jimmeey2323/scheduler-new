...
import { Clock, Users, TrendingUp, Star, Lock, Zap, Award, Calendar, Target, ChevronDown, ChevronUp, Filter, Eye, Edit, ChevronLeft, ChevronRight, AlertTriangle, Shield, EyeOff, Plus, Activity, BarChart3, PieChart, Layers } from 'lucide-react';
import { ClassData, ScheduledClass, AIRecommendation } from '../types';
import { getClassAverageForSlot, getTimeSlotsWithData, getClassesAtTimeSlot, getAvailableTimeSlots, getRestrictedTimeSlots, isTimeRestricted, ALL_TIME_SLOTS, hasHistoricalData, getBestSlotRecommendation } from '../utils/classUtils';
...
const WeeklyCalendar: React.FC<WeeklyCalendarProps> = ({
  ...
  const getHistoricData = (day: string, time: string) => {
    ...
    const avgCheckedIn = historicClasses.reduce((sum, cls) => sum + cls.checkedIn, 0) / historicClasses.length;
    const avgRevenue = historicClasses.reduce((sum, cls) => sum + cls.totalRevenue, 0) / historicClasses.length;
    const peakCheckedIn = Math.max(...historicClasses.map(cls => cls.checkedIn));
    const checkedInTotal = historicClasses.reduce((sum, cls) => sum + cls.checkedIn, 0);
    const emptyClasses = historicClasses.filter(cls => cls.checkedIn === 0).length;
    ...
    const teacherStats = historicClasses.reduce((acc, cls) => {
      if (!acc[cls.teacherName]) {
        acc[cls.teacherName] = { checkedIn: 0, count: 0 };
      }
      acc[cls.teacherName].checkedIn += cls.checkedIn;
    ...
    const formatStats = historicClasses.reduce((acc, cls) => {
      if (!acc[cls.cleanedClass]) {
        acc[cls.cleanedClass] = { checkedIn: 0, count: 0 };
      }
      acc[cls.cleanedClass].checkedIn += cls.checkedIn;
    ...
    return {
      ...
      avgCheckedIn: parseFloat(avgCheckedIn.toFixed(1)),
      ...
      topTeachers,
      topFormats,
      popularClass: historicClasses.sort((a, b) => b.checkedIn - a.checkedIn)[0]?.cleanedClass || 'N/A',
      bestTeacher: historicClasses.sort((a, b) => b.checkedIn - a.checkedIn)[0]?.teacherName || 'N/A'
    };
  };
  ...
  const renderCell = (day: string, time: string) => {
    ...
    // Apply filters
    if (historicData && historicData.avgCheckedIn < minParticipants) {
      return (
        <div
          key={`${day}-${time}`}
    ...
    return (
      ...
      >
          ...
          <div className="absolute inset-0 p-2 flex items-center justify-center">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-400">
                {historicData.avgCheckedIn.toFixed(1)} avg
              </div>
        ...
        {/* Enhanced Hover Tooltip with AI Recommendations */}
        {(historicData || scheduledClassesInSlot.length > 0 || isRestricted || (isHovered && hasHistorical)) && (
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-96 rounded-xl p-4 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none z-30 shadow-2xl bg-gray-900/95 backdrop-blur-sm text-white border border-gray-600">
            {scheduledClassesInSlot.length > 0 ? (
              <div>
                ...
                <div className="space-y-3">
                  {scheduledClassesInSlot.map((cls, index) => (
                    <div key={cls.id} className="p-3 rounded-lg bg-gray-800/80 backdrop-blur-sm border border-gray-700">
                      ...
                      {cls.isPrivate && (
                        <div className="p-2 rounded-lg mt-2 bg-purple-500/20 border border-purple-500/30">
                      ...
                      {cls.isHosted && cls.clientDetails && (
                        <div className="p-2 rounded-lg mt-2 bg-blue-500/20 border border-blue-500/30">
                          <div className="text-xs font-medium text-blue-300">
              ...
              <div>
                <div className="font-semibold mb-3 flex items-center text-white">
                  ...
                  AI Recommendations for {day} {time}
                </div>
                ...
                {loadingRecommendations ? (
                  ...
                  <div className="space-y-2 pointer-events-auto max-h-64 overflow-y-auto">
                    {slotRecommendations.map((rec, index) => (
                      <div
                        key={index}
                        onClick={() => handleRecommendationClick(rec, day, time)}
                        className="p-3 rounded-lg bg-gray-800/80 backdrop-blur-sm hover:bg-gray-700/80 cursor-pointer transition-all duration-200 border border-gray-600 hover:border-green-500 hover:shadow-lg"
                      >
                    ...
                    <div className="text-xs text-center text-gray-400 mt-2 p-2 bg-gray-800/50 rounded">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    ...
                    <div className="font-medium text-white">{historicData.count}</div>
                  </div>
                  <div>
                    <div className="text-gray-400">Avg Check-ins:</div>
                    <div className="text-green-400 font-medium">{historicData.avgCheckedIn.toFixed(1)}</div>
                  </div>
                  <div>
                    <div className="text-gray-400">Total Check-ins:</div>
                    <div className="text-blue-400 font-medium">{historicData.checkedInTotal}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    <div className="text-gray-400">Late Cancellations:</div>
                    <div className="text-orange-400 font-medium">{historicData.lateCancellations}</div>
                  </div>
                  <div>
                    <div className="text-gray-400">Comped %:</div>
                    <div className="text-purple-400 font-medium">{historicData.compsPercentage.toFixed(1)}%</div>
                  </div>
                  <div>
                  ...
                  <div>
                    <div className="text-gray-400">Peak Check-ins:</div>
                    <div className="text-blue-400 font-medium">{historicData.peakCheckedIn}</div>
                  </div>
                </div>

                <div className="grid grid-cols-1 gap-3 mb-4">
                  <div className="p-2 rounded-lg bg-blue-500/20">
                    <div className="text-xs font-medium mb-1 text-blue-300">Top 3 Teachers:</div>
                    {historicData.topTeachers.map((teacher, index) => (
                      <div key={index} className="text-xs text-blue-200">
                        {index + 1}. {teacher.teacher} ({teacher.avgCheckedIn.toFixed(1)} avg)
                      </div>
                    ))}
                  </div>
                  
                  <div className="p-2 rounded-lg bg-green-500/20">
                    <div className="text-xs font-medium mb-1 text-green-300">Best Class Formats:</div>
                    {historicData.topFormats.map((format, index) => (
                      <div key={index} className="text-xs text-green-200">
                        {index + 1}. {format.format} ({format.avgCheckedIn.toFixed(1)} avg)
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className={`backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border ${theme.card}`}>
        <div className={`p-6 border-b bg-gradient-to-r ${theme.secondary}/30`}>
          ...
          <div className="mb-4">
            <div className="flex items-center space-x-4">
              <button
                ...
                className={`flex items-center px-4 py-2 rounded-lg transition-all duration-200 ${theme.card} hover:scale-105 shadow-lg`}
              >
                <Layers className={`h-4 w-4 mr-2 ${theme.accent}`} />
                <span className={theme.text}>Class Mix</span>
            ...
            {showFilters && (
              <div className={`mt-4 p-6 rounded-xl border ${theme.card} shadow-lg`}>
                {/* Class Mix by Day */}
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {days.map(day => (
                      <div key={day} className={`p-4 rounded-xl border ${theme.card} shadow-sm hover:shadow-md transition-shadow`}>
                        <div className={`text-sm font-bold ${theme.text} mb-3 text-center`}>{day.slice(0, 3)}</div>
                        <div className="space-y-2">
                          {Object.entries(classMixByDay[day] || {}).map(([format, count]) => (
                            <div key={format} className="flex items-center justify-between text-xs">
                              <span className={`${theme.textSecondary} truncate`} title={format}>
                                {format.split(' ').slice(-1)[0]}
                              </span>
                              <span className={`ml-1 ${theme.text} font-medium px-2 py-1 rounded-full bg-blue-500/20 text-blue-300`}>
                                {count}
                              </span>
                            </div>
                          ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <h5 className={`text-sm font-medium ${theme.text} mb-3 flex items-center`}>
          ...
          <div className="flex items-center space-x-6 text-sm">
            <div className="flex items-center">
              <div className="w-3 h-3 bg-gradient-to-r from-yellow-400 to-amber-500 rounded-full mr-2 shadow-sm"></div>
              <span className={theme.textSecondary}>Top Performer</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-gradient-to-r from-purple-400 to-pink-500 rounded-full mr-2 shadow-sm"></div>
              <span className={theme.textSecondary}>Private Class</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-gradient-to-r from-indigo-400 to-blue-500 rounded-full mr-2 shadow-sm"></div>
              <span className={theme.textSecondary}>Hosted Class</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-gradient-to-r from-green-400 to-emerald-500 rounded-full mr-2 shadow-sm"></div>
              <span className={theme.textSecondary}>Regular Class</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 bg-gradient-to-r from-red-400 to-orange-500 rounded-full mr-2 shadow-sm"></div>
              <span className={theme.textSecondary}>Restricted (Private Only)</span>
            </div>
            <div className="flex items-center">
              <div className="w-3 h-3 rounded-full mr-2 bg-gradient-to-r from-blue-400 to-cyan-500 shadow-sm"></div>
              <span className={theme.textSecondary}>Historic Data (Hover for AI)</span>
            </div>
            <div className="flex items-center">
              <div className={`w-3 h-3 rounded-full mr-2 bg-gray-600 shadow-sm`}></div>
